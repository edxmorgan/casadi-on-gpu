...
def _inject_author_comment(path: Path, author: str) -> None:
    author = author.strip()
    if not author:
        return
    ...
    # Generated units and registry are C/CUDA source files.
    prefix = f"// {tag}\n\n"
...
def _generate_and_collect(
    ...
    for e in entries:
        ...
        _inject_author_comment(generated_dir / source, author)
        _inject_author_comment(generated_dir / header, author)
...
def _generate_registry_source(manifest: dict[str, Any], author: str) -> str:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Author tag inserted into generated source files (empty string disables)",
    )
    ...
    manifest = _generate_and_collect(entries, generated_dir, args.casadi_real, args.author)
    manifest_json = json.dumps(manifest, indent=2, sort_keys=False) + "\n"
    registry_src = _generate_registry_source(manifest, args.author)
